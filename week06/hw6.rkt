...
(define (install-complex-package)
  ...
  (put 'equ? '(complex complex)
       (lambda (z1 z2) (and (equal? (real-val z1) (real-val z2))
                       (equal? (imag-val z1) (imag-val z2)))
        ))
  (put '=zero? '(complex)
       (lambda (z) (and (= 0 (real-val z)) (= 0 (imag-val z)))))
      ...
      (if proc
          ...
          (if (= (length args) 2)
              (let ((type1 (car type-tags))
                ...
                (let ((t1->t2 (get-coercion type1 type2))
                      (t2->t1 (get-coercion type2 type1)))
                  (cond ((and (not (equal? type1 type2)) t1->t2)
                         (apply-generic op (t1->t2 a1) a2))
                        ((and (not (equal? type1 type2)) t2->t1)
                         (apply-generic op a1 (t2->t1 a2)))
                        (else
                         (error "No method for these types"
                                (list op type-tags)))))) 
...
(define (install-coercion)
  ...
  (define (rational->racket-number n)
    (make-racket-number (/ (car n) (cdr n))))

  (define (racket-number->complex n)
    (make-complex-from-real-imag n 0))

  (put-coercion 'raise-num '(integer) integer->rational)
  (put-coercion 'raise-num '(rational) rational->racket-number)
  (put-coercion 'raise-num '(racket-number) racket-number->complex)
  'coercion-installed
)
...
; (eval-1 '(map-1 (lambda (x) (* x x)) '(8 2 11)))
; (map-1 (lambda (x) (* x x)) '(8 2 11))
...
; (trace union-ordered-set)
; (union-ordered-set '(1 3) '(1 2 4))
;; The Binary Tree ADT
